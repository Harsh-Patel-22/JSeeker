...
public class ResumeBuilderService (AIService aiService, GithubService githubService, UserRepository userRepository) {
    ...
    public async Task GenerateFullResumeAsync(Guid userId) {
        ...
        var cleanedJson = StringCleaner.GetCleanJsonString(response);
        await userRepository.SetResumeJsonStringAsync(userId, cleanedJson);
    ...
    public async Task<JsonElement> GetResumeDescriptionAsync() {
        ...
        var cleanedResponse = StringCleaner.GetCleanJsonString(response);
